...
if((res = hal_pin_float_new("XYZACkins.Tool-offset", HAL_IN, &(haldata->Tool_offset),
comp_id)) < 0) goto error;
if((res = hal_pin_float_new("XYZACkins.Y-offset", HAL_IO, &(haldata->Y_offset),
comp_id)) < 0) goto error;
if((res = hal_pin_float_new("XYZACkins.Z-offset", HAL_IO, &(haldata->Z_offset),
comp_id)) < 0) goto error;
hal_ready(comp_id);
return 0;
error:
